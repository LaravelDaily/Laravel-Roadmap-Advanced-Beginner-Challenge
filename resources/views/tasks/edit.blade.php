    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{url('/tasks/'.$task->id)}}" method="POST">
                @csrf
                @method('PUT')
                <input name="id" value="{{old('id',$task->id)}}" type="hidden"/>
                <input name="created_by" value="{{old('id',$task->created_by)}}" type="hidden"/>
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-3 ">
                                ...
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @if(request()->project) bg-gray-400 @endif">
                                    ...
                                    @forelse($projects as $id=>$name)
                                        <option value="{{$id}}" @if(old('project_id',$task->project_id)==$id) selected @endif >
                                            {{$name}}</option>
                                    @empty
                            ...
                            <div class="col-span-6 ">
                                ...
                                <textarea name="description" id="description"
                                          autocomplete="off"
                                          class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{old('description',$task->description)}}</textarea>
